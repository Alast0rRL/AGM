...
136 тестов на чистую логику: паттерны, фильтры, хелперы. **Все тесты должны проходить до и после каждого изменения bot.py.** Если тест упал — откат.

## Эталонный сценарий

Идеальное начало диалога — так бот должен работать:
...
[Stage 3]
Бесконечное общение, отвечает на вопросы
```

**Что должно работать:**
- На "привет" отвечаем "сколько лет" (не "привет" обратно)
- На возраст 18/17/19 — принимаем, на остальное — пропуск
- На "а тебе" — отвечаем "19"
- На вопрос имени — "Максим, тебя?"
- Однословное имя (типа "Вика" или "катя") — записываем как `partner_name`
- Stage 3 запоминает триггеры (имя, город, приятно) и не повторяет ответы

### Сценарий 2: Имя с маленькой буквы
...
Partner → "как зовут"
Bot → "Максим, тебя?"
Partner → "катя"           ← маленькая буква, ОК
✓ Имя получено (катя)
Partner → "приятно"        ← followup
Bot → "взаимно"            ← отвечаем

[Stage 3]
Бесконечное общение
```

**Важно:** имена распознаются с любой буквы (катя, Вика, Аня). Слова из `_NOT_NAMES` (ладно, круто, спасибо и т.д.) не считаются именами.
...
4. **Тесты** — `python test_bot.py`, все 136 должны пройти
